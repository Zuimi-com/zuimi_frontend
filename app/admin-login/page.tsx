...
const [password, setPassword] = useState('')
 ...
 const handleForgotPassword = () => {
  ...
  const Baseurl = process.env.NEXT_PUBLIC_API_BASE_STAGING;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${Baseurl}/api/v1/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'platform': 'ADMIN' },
      ...
      if (!response.ok) {
        ...
        if (data?.data) {
          Cookies.set("authToken", data.data, {
            expires: 7,
            secure: true,
            sameSite: "strict"
          });
...
<div>
          <button
            type='submit'
            disabled={isDisabled}
            className={`inline-flex items-center justify-center rounded-md w-[752px] h-[48px] ${isDisabled ? "bg-[#1684EF] " : "bg-[#FFE600] cursor-pointer"} px-4 py-2 text-lg font-medium text-white hover:opacity-95`}
          >
            {loading ? (
              <>
